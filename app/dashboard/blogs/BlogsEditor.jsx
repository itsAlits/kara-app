...
function BlogGridCard({ blog, index, isFirst, isLast, onSelect, onMoveUp, onMoveDown, onRemove }) {
  return (
    <div onClick={onSelect} className="group relative flex flex-col rounded-2xl overflow-hidden border border-white/[0.07] bg-[#161922] hover:border-indigo-500/40 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-indigo-900/10">
      {/* Cover */}
      <div className="relative w-full h-40 bg-slate-800">
        {blog.coverImage
          ...
          : <div className="w-full h-full flex items-center justify-center opacity-30">
              <svg className="w-8 h-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}><path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/></svg>
            </div>
        }
        {/* overlay */}
        ...
        <div className="absolute top-2 left-2">
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${blog.published ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-600/40 text-slate-400"}`}>
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        <p className="text-sm font-semibold text-white truncate">{blog.title || <span className="text-slate-500 italic font-normal">Untitled</span>}</p>
        <div className="flex items-center gap-2">
          {blog.category && <span className="text-[10px] text-slate-500">{blog.category}</span>}
          {blog.author && <span className="text-[10px] text-slate-600 ml-auto truncate">{blog.author}</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div
        ...
        className={`mt-2 relative h-36 rounded-xl border-2 border-dashed cursor-pointer transition-all duration-200 overflow-hidden flex items-center justify-center ${dragging?"border-indigo-500 bg-indigo-600/10":"border-slate-700 hover:border-slate-500 bg-[#0d0f14]"}`}
      >
      ...
      <div className="flex items-center gap-3 pb-4 mb-4 border-b border-white/[0.06]">
        ...
        <div className="min-w-0 flex-1 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={`shrink-0 flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-200 ${
              blog?.published ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/20" : "bg-slate-700/40 text-slate-400 border-slate-600/30 hover:bg-slate-700/60"
            }`}
          >
